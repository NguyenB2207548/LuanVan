...
const AiLayerModal: React.FC<AiLayerModalProps> = ({
  isOpen,
  onClose,
  onApply,
}) => {
  const [file, setFile] = useState<File | null>(null);
  const [previewUrl, setPreviewUrl] = useState<string | null>(null); // State mới để chứa link ảnh preview
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (selectedFile) {
      // Tạo link preview tạm thời cho ảnh vừa chọn
      setPreviewUrl(URL.createObjectURL(selectedFile));
      setResult(null); // Xóa kết quả cũ (nếu có) khi người dùng đổi ảnh mới
    } else {
      setPreviewUrl(null);
    }
  };
  ...
  const handleApply = () => {
    ...
    const scaleDown = 0.6;
    const targetSize = 650 * scaleDown;
            ...
            {loading ? (
              <div className="flex flex-col items-center gap-4">
                ...
                <p className="text-sm font-medium animate-pulse text-gray-500">
                  AI đang phân tích và tách lớp...
                </p>
              </div>
            ) : result ? (
              // HIỂN THỊ KẾT QUẢ AI ĐÃ TÁCH
              <div className="grid grid-cols-3 gap-3 w-full">
                {result.layers.map((l: any, idx: number) => (
                  <div
                    key={idx}
                    className="bg-white p-2 border rounded shadow-sm relative group"
                  >
                    <img
                      src={`http://localhost:3000${l.publicUrl}`}
                      className="w-full aspect-square object-contain bg-gray-200 rounded"
                      alt="layer"
                    />
                    <span className="text-[10px] block mt-1 text-center font-medium truncate">
                      {l.label}
                    </span>
                  </div>
                ))}
              ...
              <div className="flex flex-col items-center">
                ...
                <img
                  src={previewUrl}
                  alt="Preview"
                  className="max-w-full max-h-[300px] object-contain rounded-md shadow-sm border border-gray-200 bg-white"
                />
              </div>
            ) : (
              ...
              <div className="text-gray-400 text-center">
                ...
                <p className="text-sm">Vui lòng chọn ảnh để bắt đầu</p>
              </div>
            )}
          </div>
        </div>

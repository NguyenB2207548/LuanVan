...
interface OrderDetailModalProps {
  ...
  isOpen: boolean;
  onClose: () => void;
  onOpenReview: (item: any, displayImg: string) => void;
}
        ...
        <div className="p-8 space-y-9">
          ...
          <section>
            <h3 className="flex items-center gap-2 text-sm font-semibold text-gray-800 mb-6">
              <Package size={18} className="text-gray-400" /> Danh sách sản phẩm
              mua
            </h3>
            <div className="grid grid-cols-1 gap-6">
              {order.items.map((item: any) => {
                // 👇 1. XỬ LÝ ẢNH ĐẠI DIỆN Ở ĐÂY
                const variantImg = item.variant?.images?.find(
                  (img: any) => img.isPrimary,
                )?.url;
                const productImg = item.variant?.product?.images?.[0]?.url;
                const fallbackImg = variantImg || productImg;

                // Ưu tiên Base64, không có thì xài ảnh gốc
                const displayImg =
                  item.previewDesign ||
                  (fallbackImg ? `${BASE_URL}${fallbackImg}` : "");

                return (
                  <div
                    key={item.id}
                    className="flex gap-6 items-center bg-gray-50/30 p-4 rounded-sm border border-gray-50"
                  >
                    <div className="w-20 h-20 bg-white border border-gray-100 rounded overflow-hidden shrink-0">
                      {/* 👇 2. GẮN ẢNH VÀO THẺ IMG */}
                      <img
                        src={displayImg}
                        alt="product"
                    ...
                    <div className="flex-1 min-w-0 grid grid-cols-1 md:grid-cols-2 gap-4 items-center">
                      <div>
                        ...
                        <p className="text-xs text-gray-400 mt-1">
                          Phân loại: {item.variantNameSnapshot}
                        </p>
                      </div>
                      <div className="flex justify-between md:justify-end md:gap-12 items-center">
                        ...
                        {order.status === "success" && (
                          <button
                            // 👇 3. TRUYỀN THÊM displayImg VÀO HÀM NÀY NỮA
                            onClick={() => onOpenReview(item, displayImg)}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
            <section>
              <h3 className="flex items-center gap-2 text-sm font-semibold text-gray-800 mb-4">
                <MapPin size={18} className="text-gray-400" /> Thông tin người
                nhận
              </h3>
              <div className="text-sm space-y-2 text-gray-600">
                ...
                <p className="leading-relaxed bg-gray-50 p-3 rounded-sm border border-gray-50">
              ...
              {order.shipper ? (
                <div className="text-sm space-y-2 text-gray-600">
                  ...
                  {order.shipper.shipperProfile && (
                    <p className="text-xs text-gray-400 inline-block bg-gray-100 px-2 py-1 rounded">
                      Biển số: {order.shipper.shipperProfile.vehiclePlate}
                    </p>
                  )}
                </div>
              ) : (
                <div className="h-24 flex items-center justify-center border-2 border-dashed border-gray-100 rounded">
                  <p className="text-sm text-gray-400 italic">
                    Hệ thống đang điều phối shipper...
                  </p>
                </div>
              )}
            </section>
            ...
            <div className="flex items-center justify-between bg-gray-50 p-5 rounded border border-gray-100">
              ...
              <div className="text-right">
                ...
                <p className="text-base font-bold text-amber-500">
                  {order.seller?.sellerProfile?.rating || "N/A"} ★
                </p>
              </div>
            </div>
          </section>

...
import {
  Loader2, Package, Palette, CheckCircle2, Save,
  ChevronDown, ImageIcon, Layers,
} from "lucide-react";
...
const AddDesignPage = () => {
  ...
  const getActivePrintArea = () => {
    if (selectedVariant?.mockup?.printArea) return selectedVariant.mockup.printArea;
  ...
  const handleSelectVariant = (variant: any) => {
    ...
    const printArea = variant.mockup?.printArea || selectedProduct?.mockup?.printArea;
    applyPrintArea(printArea || null);
  };
    ...
    <div className="flex h-screen bg-gray-100 overflow-hidden">
      ...
      <div className="w-80 bg-white border-r border-gray-200 flex flex-col shrink-0 z-10">
        ...
        <div className="px-5 py-4 border-b border-gray-100">
          ...
          <p className="text-xs text-gray-400 mt-0.5">Gắn artwork vào sản phẩm phôi</p>
        </div>
        ...
        <div className="flex-1 overflow-y-auto">
          ...
          <div className="px-5 py-4 border-b border-gray-100">
            ...
            <div className="space-y-1.5">
              {products.map(p => {
                const thumb = p.images?.find((i: any) => i.isPrimary)?.url
                  || p.mockup?.url || null;
                ...
                return (
                  <button
                    ...
                    className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-lg border text-left transition-colors ${isSelected
                      ? "border-gray-900 bg-gray-900 text-white"
                      : "border-gray-200 bg-white hover:border-gray-400 text-gray-700"
                      }`}
                  >
                    <div className={`w-9 h-9 rounded-lg overflow-hidden shrink-0 border ${isSelected ? "border-white/20" : "border-gray-100"} bg-gray-50 flex items-center justify-center`}>
                      {thumb
                        ? <img src={`${BASE_URL}${thumb}`} className="w-full h-full object-contain" alt="" />
            ...
            <div className="px-5 py-4 border-b border-gray-100">
              ...
              <div className="grid grid-cols-3 gap-1.5">
                ...
                <button
                  ...
                  className={`flex flex-col items-center p-2 rounded-lg border text-center transition-colors ${!selectedVariant
                    ? "border-gray-900 bg-gray-900 text-white"
                ...
                >
                  <div className="w-10 h-10 rounded-lg border border-gray-100 bg-gray-50 overflow-hidden mb-1.5 flex items-center justify-center">
                    {selectedProduct.mockup?.url
                      ? <img src={`${BASE_URL}${selectedProduct.mockup.url}`} className="w-full h-full object-contain" alt="" />
                      : <ImageIcon size={12} className="text-gray-300" />
                    }
                  </div>
                  <p className={`text-[10px] leading-tight ${!selectedVariant ? "text-white/80" : "text-gray-500"}`}>Mặc định</p>
                </button>

                {variants.map((v: any) => {
                  const vThumb = v.mockup?.url
                  ...
                  return (
                    <button
                      ...
                      className={`flex flex-col items-center p-2 rounded-lg border text-center transition-colors ${isSelected
                        ? "border-gray-900 bg-gray-900 text-white"
                        : "border-gray-200 hover:border-gray-400 text-gray-600"
                        }`}
                    >
                      <div className="w-10 h-10 rounded-lg border border-gray-100 bg-gray-50 overflow-hidden mb-1.5 flex items-center justify-center">
                        {vThumb
                          ? <img src={`${BASE_URL}${vThumb}`} className="w-full h-full object-contain" alt="" />
                          : <ImageIcon size={12} className="text-gray-300" />
                        }
                      </div>
                      <p className={`text-[10px] leading-tight truncate w-full ${isSelected ? "text-white/80" : "text-gray-500"}`}>
                        {v.sku || `#${v.id}`}
                      </p>
                    </button>
                  );
          ...
          <div className="px-5 py-4">
            <p className="text-[10px] font-semibold text-gray-400 uppercase tracking-wider flex items-center gap-1.5 mb-3">
              <Palette size={11} /> {selectedProduct && variants.length > 0 ? "Bước 3" : "Bước 2"} · Chọn artwork
            </p>
            {artworks.length === 0 ? (
              <p className="text-xs text-gray-400 italic">Chưa có artwork nào</p>
            ) : (
              <div className="space-y-1.5">
                {artworks.map(art => {
                  ...
                  return (
                    ...
                    >
                      <div className={`w-8 h-8 rounded-lg shrink-0 flex items-center justify-center ${isSelected ? "bg-white/10" : "bg-gray-100"}`}>
                        <Palette size={14} className={isSelected ? "text-white/70" : "text-gray-400"} />
                      </div>
                      <div className="flex-1 min-w-0">
                        <p className={`text-sm font-medium truncate ${isSelected ? "text-white" : "text-gray-800"}`}>
                          {art.artworkName || `Artwork #${art.id}`}
                        </p>
                        <p className={`text-xs mt-0.5 ${isSelected ? "text-white/50" : "text-gray-400"}`}>
                          {art.layersJson?.details?.length || 0} layers
                        </p>
                      </div>
                      {isSelected && <CheckCircle2 size={15} className="text-white shrink-0" />}
                    </button>
                  );
                })}
              </div>
        ...
        <div className="px-5 py-4 border-t border-gray-100">
          <button
            ...
            onClick={handleSave}
            className={`w-full py-2.5 rounded-lg text-sm font-medium flex items-center justify-center gap-2 transition-colors ${saveSuccess
          ...
          <div className="flex items-center gap-4 text-sm text-gray-500">
            {selectedProduct && (
              <span className="flex items-center gap-1.5">
                ...
                <span className="text-gray-700 font-medium">{selectedProduct.productName}</span>
              </span>
            )}
            {selectedProduct && selectedArtwork && <span className="text-gray-300">/</span>}
            {selectedArtwork && (
              <span className="flex items-center gap-1.5">
                <Palette size={14} className="text-gray-400" />
                <span className="text-gray-700 font-medium">{selectedArtwork.artworkName}</span>
              </span>
            )}
            {selectedVariant && (
              <>
            ...
            {!selectedProduct && !selectedArtwork && (
              <span className="text-gray-400">Chọn sản phẩm và artwork để bắt đầu</span>
            )}
          </div>
            ...
            <div className="absolute inset-0 flex items-center justify-center bg-gray-100/80 backdrop-blur-[2px]">
              <div className="text-center">
                ...
                <p className="text-sm font-medium text-gray-500">
                  {!selectedProduct ? "Chọn sản phẩm phôi" : "Chọn artwork thiết kế"}
                </p>

...
const EditArtworkPage = () => {
    ...
    useEffect(() => {
        const fetchArtworkDetail = async () => {
            try {
                ...
                // Sử dụng data.artworkName vì JSON trả về key này
                setArtworkName(data.artworkName || "");

                // Sử dụng data.mockupUrl
                setBackgroundUrl(data.mockupUrl || "");

                // Dữ liệu layers từ Service đã là Object, không cần parse nếu đã là JSON
                if (data.layers) {
                    setLayers(Array.isArray(data.layers) ? data.layers : JSON.parse(data.layers));
                }
                ...
                toast.error("Không thể tải thông tin Artwork.");
    ...
    const handleUpdateArtwork = async (payload: any) => {
        try {
            setLoading(true);
            // Payload gửi lên backend cần khớp với DTO (thường là artworkName và layersJson)
            await axiosClient.patch(`designs/seller/artworks/${id}`, payload);
            toast.success("Cập nhật Artwork thành công!");

            setTimeout(() => navigate("/seller/artworks"), 1500);
        } catch (err) {
            console.error("Lỗi khi cập nhật:", err);
            toast.error("Lỗi khi cập nhật Artwork!");
        } finally {
            setLoading(false);
        }
    };
    ...
    if (loading && layers.length === 0)
        return (
            <div className="flex h-screen flex-col items-center justify-center bg-[#F8F9FA] gap-3">
                ...
                <span className="text-sm font-medium text-gray-500">Đang đồng bộ dữ liệu thiết kế...</span>
            </div>
        );

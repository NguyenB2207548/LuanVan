...
import {
    ...
    ChevronDown, ImageIcon, Layers, ArrowLeft
} from "lucide-react";
    ...
    useEffect(() => {
        const fetchInitialData = async () => {
            try {
                ...
                const foundProduct = allProducts.find((p: any) => p.id === currentDesign.product?.id);
                const foundArtwork = allArtworks.find((a: any) => a.id === currentDesign.artwork?.id);
    ...
    const handleUpdate = async () => {
        ...
        try {
            await axiosClient.patch(`/designs/${id}`, {
                designName: designName || `${selectedProduct.productName} - ${selectedArtwork.artworkName}`,
            ...
            toast.error("Lỗi khi cập nhật thiết kế.");
        } finally {
            setSaving(false);
        }
    };
    ...
    if (loading) return (
        <div className="flex h-screen items-center justify-center bg-[#F8F9FA]">
            <div className="flex flex-col items-center gap-3">
                ...
                <span className="text-sm font-medium text-gray-500">Đang tải cấu hình thiết kế...</span>
            </div>
        </div>
    );
        ...
        <div className="flex h-screen bg-[#F8F9FA] overflow-hidden">
            ...
            <div className="w-80 bg-white border-r border-gray-200 flex flex-col shrink-0 z-10 shadow-sm">
                ...
                <div className="px-5 py-4 border-b border-gray-100 flex items-center gap-3">
                    ...
                    <div>
                        <h1 className="text-base font-bold text-gray-900">Chỉnh sửa thiết kế</h1>
                        <p className="text-[10px] text-gray-400">Cập nhật sự kết hợp sản phẩm & artwork</p>
                    </div>
                </div>
                ...
                <div className="flex-1 overflow-y-auto custom-scrollbar">
                    ...
                    <div className="px-5 py-4 border-b border-gray-100">
                        <label className="text-[10px] font-bold text-gray-400 uppercase tracking-widest block mb-2">Tên hiển thị</label>
                        <input
                    ...
                    <div className="px-5 py-4 border-b border-gray-100">
                        ...
                        <div className="space-y-1.5">
                            {products.map(p => {
                                const thumb = p.images?.find((i: any) => i.isPrimary)?.url || p.mockup?.url;
                                const isSelected = selectedProduct?.id === p.id;
                                return (
                                    <button
                                        key={p.id}
                                        onClick={() => handleSelectProduct(p)}
                                        className={`w-full flex items-center gap-3 px-3 py-2 rounded-lg border text-left transition-all ${isSelected ? "border-blue-600 bg-blue-50/50 shadow-sm" : "border-gray-100 bg-white hover:border-gray-300"
                                            }`}
                                    >
                                        <div className="w-10 h-10 rounded border border-gray-100 bg-white overflow-hidden shrink-0">
                                            {thumb ? <img src={`${BASE_URL}${thumb}`} className="w-full h-full object-contain" /> : <ImageIcon size={14} className="text-gray-300" />}
                                        </div>
                                        <span className={`text-sm font-medium truncate flex-1 ${isSelected ? "text-blue-700" : "text-gray-700"}`}>{p.productName}</span>
                        ...
                        <div className="space-y-1.5">
                            {artworks.map(art => {
                                ...
                                return (
                                    ...
                                    >
                                        <div className={`w-8 h-8 rounded shrink-0 flex items-center justify-center ${isSelected ? "bg-blue-100" : "bg-gray-50"}`}>
                                            <Palette size={14} className={isSelected ? "text-blue-600" : "text-gray-400"} />
                                        </div>
                                        <span className={`text-sm font-medium truncate flex-1 ${isSelected ? "text-blue-700" : "text-gray-700"}`}>{art.artworkName}</span>
                                        {isSelected && <CheckCircle2 size={16} className="text-blue-600" />}
                                    </button>
                                );
                            })}
            ...
            <div className="flex-1 flex flex-col">
                <div className="h-12 bg-white border-b border-gray-200 flex items-center px-5 gap-4">
                    <span className="text-xs font-bold text-blue-600 bg-blue-50 px-2 py-1 rounded">Chế độ chỉnh sửa</span>
                    <span className="text-sm text-gray-500 font-medium truncate">{designName}</span>
                </div>

...
import { RotateCcw, ArrowLeft, Maximize, Loader2 } from "lucide-react";
...
const PrintAreaConfigPage = () => {
  ...
  const handleSaveConfig = async () => {
    ...
    try {
      ...
      showSuccessToast("Đã cập nhật vùng in");
      navigate(-1);
    } catch (err) {
      showErrorToast("Lỗi cập nhật");
    }
  };
    ...
    <div className="flex flex-col h-screen bg-white text-gray-800">
      <div className="h-12 bg-gray-100 border-b px-4 flex items-center justify-between z-10">
        <div className="flex items-center gap-3">
          <button
            onClick={() => navigate(-1)}
            className="p-1 hover:bg-gray-200 border rounded-sm"
          >
            <ArrowLeft size={18} />
          </button>
          <h1 className="text-xs font-bold uppercase">
            Vùng In: <span className="text-blue-600">{targetName}</span>
          </h1>
        </div>
        <div className="flex items-center gap-2">
          <button
            onClick={handleReset}
            className="px-3 py-1.5 text-xs font-bold text-gray-600 border rounded-sm hover:bg-gray-200"
          >
            <RotateCcw size={14} /> Reset
          </button>
          <button
            onClick={handleSaveConfig}
            className="bg-blue-700 text-white px-4 py-1.5 rounded-sm font-bold text-xs uppercase shadow-sm"
          >
            Xác nhận & Quay lại
          </button>
        </div>
          ...
          <section>
            ...
            <div className="grid grid-cols-1 gap-3">
              {["x", "y", "width", "height"].map((key) => (
                <div
                  key={key}
                  className="flex items-center justify-between bg-white border p-2 rounded-sm shadow-sm"
                >
                  ...
                  <input
                    ...
                    value={Math.round(
                      virtualPrintArea[
                        key as keyof typeof DEFAULT_PRINT_AREA
                      ] as number,
                    )}

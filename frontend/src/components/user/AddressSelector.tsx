...
const AddressSelector = ({ onAddressChange }: AddressSelectorProps) => {
    ...
    const handleProvinceChange = async (e: React.ChangeEvent<HTMLSelectElement>) => {
        ...
        const provinceName = provinces.find((p) => String(p.code) === provinceCode)?.name || "";
    ...
    const handleDistrictChange = async (e: React.ChangeEvent<HTMLSelectElement>) => {
        ...
        const districtName = districts.find((d) => String(d.code) === districtCode)?.name || "";
    ...
    const selectClass = "w-full bg-gray-50 border border-gray-200 text-gray-600 text-sm rounded-md p-2.5 outline-none focus:border-gray-400 transition-all cursor-pointer disabled:bg-gray-100 disabled:cursor-not-allowed";
    ...
    return (
        <div className="space-y-4 font-sans">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                {/* Phường/Xã */}
                <select
                    onChange={(e) => setSelectedWard(wards.find(w => String(w.code) === e.target.value)?.name || "")}

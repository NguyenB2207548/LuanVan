...
import {
  ...
  LayoutDashboard,
  ChevronDown,
  Package, // Thêm icon cho Lịch sử đơn hàng
  History,
} from "lucide-react";
import {
  DropdownMenu,
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-20 flex items-center justify-between">
        ...
        <div className="flex items-center gap-8">
          ...
          <nav className="hidden md:flex items-center gap-6">
            ...
            {/* DROPDOWN: HỢP TÁC (Dành cho khách chưa đăng nhập hoặc xem nhanh) */}
            <DropdownMenu>
          ...
          {isAuthenticated && user ? (
            <DropdownMenu>
              ...
              >
                ...
                {/* --- PHẦN QUẢN LÝ DÀNH RIÊNG CHO TỪNG ROLE --- */}
                ...
                {/* Nếu là ADMIN (Dùng chung icon Dashboard) */}
                {user.role === "admin" && (
                  <DropdownMenuItem
                ...
                {/* Link đăng ký đối tác (Chỉ hiện nếu đang là User thường) */}
                {user.role === "user" && (
                  <>
                    <DropdownMenuItem
                      onClick={() => navigate("/register-seller")}
                      className="gap-3 py-3 cursor-pointer rounded-xl text-slate-600"
                    >
                      <Store size={18} />
                      <span className="font-semibold text-sm">
                        Đăng ký bán hàng

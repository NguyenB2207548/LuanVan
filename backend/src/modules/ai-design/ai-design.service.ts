...
export class AiDesignService {
  ...
  private async warmUpModel() {
    try {
      console.log('Bắt đầu warm up model qwen-image-layered...');
      await fal.subscribe('fal-ai/qwen-image-layered', {
        input: {
          image_url: 'https://fal.media/files/rabbit/rmgBPTMYnhUOJnCnFHOuj.png',
          prompt: 'test',
          num_layers: 2,
        ...
        onQueueUpdate: (update: any) => {
          if (update.status === 'IN_PROGRESS') {
            console.log('[WarmUp Queue] Đang khởi động model...');
          }
        },
      });
      console.log('Warm up model thành công!');
    } catch (error) {
      console.error('Error occurred while warming up the model:', error);
    }
  }
    ...
    try {
      ...
      const result = await fal.subscribe('fal-ai/qwen-image-layered', {
        input: {
          ...
          prompt:
            prompt ||
            'Segment the image into separate high-quality transparent layers',
      ...
      throw new InternalServerErrorException(`Lỗi xử lý AI`);
    }
  }
}
